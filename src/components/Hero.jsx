...
// components/Hero.js
import React from "react";
import { containerClass } from "@/utils/css-utils"; // Replace or remove if not using utility
...
export default function Hero({
  ...
  return (
    <section className="relative bg-red-100 w-full h-auto min-h-[400px] md:min-h-[500px]">
      <div className={`${containerClass} grid grid-cols-1 md:grid-cols-2 py-10 items-center  w-full h-full`}>
        <div className="flex flex-col space-y-6">
          <h1 className="text-3xl md:text-4xl lg:text-5xl font-semibold text-center md:text-left whitespace-pre-line">
            {title}
          </h1>

          {subtitle && (
            <p className="text-center md:text-left text-gray-700">{subtitle}</p>
          )}

          {(primaryBtnText || secondaryBtnText) && (
            <div className="flex gap-3 justify-center md:justify-start">
              {primaryBtnText && (
                <a href={primaryBtnLink} className="h-10 px-4 text-sm rounded-md bg-red-600 text-white flex items-center justify-center">
                  {primaryBtnText}
                </a>
              )}
              {secondaryBtnText && (
                <a href={secondaryBtnLink} className="h-10 px-4 text-sm rounded-md border border-red-600 text-red-600 flex items-center justify-center">
                  {secondaryBtnText}
                </a>
              )}
            </div>
        ...
        {image && (
          <div className="hidden md:flex h-full w-full overflow-hidden justify-center">
            <img src={image} alt="hero" className="object-cover h-full" />
          </div>
        )}
      </div>
    </section>
